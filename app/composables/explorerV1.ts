...
import type { Group, Course } from "~/utils/types/entities";
import {
...
import { isNotUndefString } from "~/utils/guards";
...
export async function useExplorerV1Courses(
	...
	const { explorerV1CoursesURL = "", explorerV1CoursesPath = "" } = CUNA.config || {};
	const coursesEndpoint = `${explorerV1CoursesURL}${explorerV1CoursesPath}`;
	const query: ExplorerV1CoursesPayload = {
		nivel: values.level,
		sede: values.place,
	...
	const response = await useQuery<ExplorerV1CoursesResponse>(coursesEndpoint, { query });
	const codes: string[] = [];
