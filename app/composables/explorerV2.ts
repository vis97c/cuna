...
import type { Course } from "~/utils/types/entities";
import {
	eSIATypology,
...
export async function useExplorerV2Courses(values: CourseValues): Promise<CoursesResponse<Course>> {
	...
	const endpoint = CUNA.config?.explorerV2CoursesURL;
	...
	if (values.code) {
		...
		courses = await useQuery<ExplorerV2Course[]>(coursesEndpoint);
	} else {
		...
		const faculties = await useQuery<ExplorerV2Faculty[]>(facultiesEndpoint);
		...
		const programs = await useQuery<ExplorerV2Program[]>(programsEndpoint);
