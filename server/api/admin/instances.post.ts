...
import { eMemberRole } from "~~/functions/src/types/entities/member";

/**
 * Get the edges from the "instances" collection.
 *
 * @auth assistant
 * @order createdAt
 */
export default defineConditionallyCachedEventHandler(async (event) => {
	const { firebaseFirestore } = getServerFirebase("api:admin:instances");
	...
	try {
		...
		debugFirebaseServer(event, "api:admin:instances");
		...
		const query: CollectionReference | Query = firebaseFirestore.collection("instances");
		const resolver = new QueryResolver(event, query);

		return resolver.resolve();
	} catch (err) {
		apiLogger(event, "api:admin:instances", err);
