...
/**
 * Get the edges from the "proxies" collection.
 *
 * @auth assistant
 * @order createdAt
 */
export default defineConditionallyCachedEventHandler(async (event) => {
	const { firebaseFirestore } = getServerFirebase("api:admin:instance:proxies");
	...
	try {
		...
		apiLogger(event, "api:admin:instance:proxies", err);
