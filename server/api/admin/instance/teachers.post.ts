...
/**
 * Get the edges from the teachers collection
 */
export default defineConditionallyCachedEventHandler(async (event) => {
	...
	try {
		...
		const query: Query<CourseData> = currentInstanceRef.collection("teachers");
		const resolver = new QueryResolver(event, query);

		return resolver.resolve();
	} catch (err) {
		apiLogger(event, "api:instance:teachers", err);
